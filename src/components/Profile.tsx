...
export default function Profile({ profile: initialProfile, partnerProfile, onLogout }: ProfileProps) {
  ...
  const handleLinkPartner = async () => {
    ...
    try {
      ...
      alert("Fehler bei der Verknüpfung.");
    } finally {
      setIsLinking(false);
    }
  };
  ...
  return (
    <div className="flex flex-col h-full animate-entrance overflow-visible">
      ...
      <div className="flex-1 flex flex-col pt-1 w-full overflow-visible">
        <div className="flex items-center justify-between mb-4 pt-1 relative min-h-[40px]">
          ...
          <div className="text-center w-full px-4">
            {isEditingName ? (
              <div className="flex items-center justify-center gap-2">
                <input type="text" value={newName} onChange={(e) => setNewName(e.target.value)} className="text-lg font-bold text-[var(--text-main)] bg-purple-50 border-b-2 border-[var(--secondary)] outline-none text-center py-0.5 w-full max-w-[160px]" autoFocus onBlur={handleUpdateName} onKeyDown={(e) => e.key === 'Enter' && handleUpdateName()} />
                <button onClick={handleUpdateName} className="text-[var(--accent-green)]"><Check className="w-5 h-5" /></button>
              </div>
            ) : (
              <div className="flex items-center justify-center gap-2 group cursor-pointer" onClick={() => setIsEditingName(true)}>
                <h2 className="text-xl font-bold text-[var(--text-main)] leading-tight">{profile?.display_name || 'User'}</h2>
        ...
        <div className="space-y-2.5 w-full overflow-hidden">
          <div className="status-box p-3.5">
            <h3 className="text-[8px] font-bold text-[var(--muted)] uppercase tracking-[0.2em] mb-2 px-1">Mein Partner-Code</h3>
          ...
          <div className="status-box p-3.5">
            <div className="flex items-center gap-2.5 mb-3 px-1">
              <Download className="w-3.5 h-3.5 text-[var(--secondary)]" />
              <h3 className="font-bold text-[9px] text-[var(--muted)] uppercase tracking-widest">App Nutzung ✨</h3>
            </div>
            {isStandalone ? (
              <div className="flex items-center gap-2 text-xs font-bold text-green-700 bg-green-50 p-3 rounded-xl border border-green-100 w-full justify-center">
                <Check className="w-4 h-4" /> Installiert & Startbereit
              </div>
            ) : isDesktop ? (
        ...
        <div className="mt-auto pb-6 pt-2 w-full relative z-20">
          <button onClick={onLogout} className="w-full p-4 rounded-[22px] border-2 border-red-100 text-red-600 font-bold flex items-center justify-center gap-2 bg-white text-xs transition-all active:scale-95 hover:border-red-200 shadow-sm">
